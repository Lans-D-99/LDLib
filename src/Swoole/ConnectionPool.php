...
class ConnectionPool {
    ...
    private function fillForLevel(int $level, float $timeout=0.5) {
        ...
        if ($this->busyGoingToLevel >= 0 || $level == $this->capacityLevel) return;
        $this->busyGoingToLevel = $level;

        $capacity = $this->capacities[$level];
        Logger::log(LogLevel::INFO,'ConnectionPool',"Filling to level $level (capacity=$capacity).");

        $nFailures = 0;
        while ($this->pool->length() < $capacity) {
            if (!$this->addNewConnection($timeout)) {
                if (++$nFailures == 10) {
                    Logger::log(LogLevel::ERROR, 'ConnectionPool', "Failed filling to level $level (capacity=$capacity).");
